...
if ($count !=0)
	{
		...
		dbQuery("INSERT INTO tbl_history (h_user, h_date, h_history)
		          VALUES ('$userName', NOW(), 'Made an appointment: $diagnosis')");
...
header('Location: index.php?view=toursms&doctor='.$doctor.'&date='.$txtDate.'&teacher='.$teacher.'&course='.$course.'&level='.$level.'&section='.$section);				  
	}
	else
		{
