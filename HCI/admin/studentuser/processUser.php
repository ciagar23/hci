...
function addUser()
{
	...
	if (dbNumRows($result) == 1) {
		...
		$sql   = "INSERT INTO tbl_user (user_fname,user_lname,user_name, user_password, user_position, user_regdate,user_phonenumber, user_course, user_level, user_section)
		          VALUES ('$FName','$LName','$userName', PASSWORD('$Password'),'$Position', NOW() ,'$Phone' ,'$Major' ,'$Level' ,'$Section')";
	
		dbQuery($sql);
		header('Location: ../user/index.php?view=thankyou&phone='.$Phone.'&user='.$userName.'&name='.$FName.' '.$LName);		
	}
}
